...
export function SquareButton({ topIcon, size = 'DEFAULT', onClick, disabled, title }: Props) {
  return (
    <div
    ...
    >
      {disabled && <div className='absolute bg-slate-100 bg-opacity-80 w-full h-full p-1' />}
      <Image alt='CarLogo' width={40} height={40} src={topIcon} />
      <Button disabled={disabled} buttonType='SMALL' shape='textOnly' textSize='text-sm'>
        {title}
      </Button>
    </div>
  );
