...
export function Button({
  ...
  return (
    <button
      disabled={disabled}
      className={classnames('flex items-center justify-center relative rounded-full', textSize, {
    ...
    >
      {leftIcon && (
        <Image className='absolute left-0 ml-1 mt-0.5' src={leftIcon} alt='arrowLeft' width={10} height={10} />
      )}
      {children}
      {rightIcon && (
        <Image className='absolute right-0 mr-1 mt-0.5' src={rightIcon} alt='arrowLeft' width={10} height={10} />
      )}
    </button>
  );
}
